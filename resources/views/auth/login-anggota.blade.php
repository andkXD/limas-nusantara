...
<div class="flex w-full min-h-screen">
  ...
  <div class="w-full lg:w-1/2 flex items-center justify-center p-gutter md:p-xl bg-background">
    <div class="w-full max-w-[420px] bg-surface-container-lowest rounded-xl shadow-[0px_12px_24px_rgba(0,0,0,0.1)] p-lg border border-outline-variant/30">
      ...
      <div class="mb-lg">
        <h2 class="font-headline-md text-headline-md text-on-surface mb-xs">Sign In Anggota</h2>
      ...
      <form class="space-y-md" method="POST" action="{{ route('anggota.login.submit') }}">
        @csrf
        <div>
          ...
          <div class="relative">
            ...
            <input class="block w-full pl-10 pr-sm py-[10px] bg-surface-container-low border border-outline-variant rounded-lg font-body-md text-body-md text-on-surface placeholder-outline focus:ring-2 focus:ring-primary focus:border-primary transition-shadow duration-200" id="email" name="email" placeholder="nama@email.com" required type="email" value="{{ old('email') }}"/>
          </div>
        </div>
        <div>
        ...
        <p class="text-center text-body-sm text-on-surface-variant pt-sm">
          Admin? <a href="{{ route('admin.login') }}" class="text-primary hover:underline">Login di sini</a>
        </p>
      </form>
    </div>
  </div>
